...
import { MangaSearch } from '@/components/hooks/UseApiFetch'
...
export default function Page() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (name) {
        ...
        if (response) {
          setSearchResults(response.mangaList || []);
  ...
  const handleSearch = async (e) => {
    e.preventDefault()
    if (searchTerm.trim()) {
      const response = await MangaSearch(searchTerm)
      if (response) {
        setSearchResults(response.mangaList || [])
      }
    }
            ...
            <div className="flex flex-col items-center space-y-4 text-center">
              <div className="space-y-2">
                ...
                <p className="mx-auto max-w-[700px] text-muted-foreground md:text-xl">
                  Temukan lebih banyak anime yang menarik dan populer
                </p>
              </div>
            ...
            >
              <AnimatePresence>
                {searchResults.length > 0 ? (
                      ...
                      <Card className="h-full overflow-hidden border-0 bg-black/40 backdrop-blur-sm">
                        <CardHeader className="relative p-0">
                          <div className="aspect-[3/3] overflow-hidden">
                            <img 
                              src={manga.image || "/placeholder.svg?height=300&width=200"} 
                              alt={manga.title} 
                        ...
                        <CardContent className="p-3">
                          <CardTitle className="text-sm font-bold text-white line-clamp-1">{manga.title}</CardTitle>
                          <div className="flex items-center space-x-2 mt-1">
                            <Star className="w-3 h-3 text-yellow-400" />
                            <span className="text-xs font-medium text-white/80">{'N/A'}</span>
                          </div>
                        </CardContent>
                        <CardFooter className="p-3 pt-0">
                          <div className="flex flex-wrap gap-1">
                            {(['Action', 'Adventure']).slice(0, 2).map((genre, index) => (
                  ...
                  <p className="col-span-full text-center text-lg text-muted-foreground">Tidak Menemukan Apapun</p>
                )}
              </AnimatePresence>
            </motion.div>
          </div>
